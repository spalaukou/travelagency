...
<c:choose>
    <c:when test="${sessionScope.login != null}"><jsp:include page="header/userHeader.jsp"/></c:when>
    <c:otherwise><jsp:include page="header/guestHeader.jsp"/></c:otherwise>
</c:choose>
...
<table cellspacing="0" cellpadding="4" border="1">
    ...
    <c:forEach var="tour" items="${tours}" varStatus="status">
        <tr>
            ...
            <td><c:out value="${tour.cost} * ${tour.hot}"/></td>
            <td>

                <a href="start?command=buy_tour&id=${tour.id}" class="button"><button>Add to cart</button></a>
            </td>
        </tr>
    </c:forEach>
</table>
